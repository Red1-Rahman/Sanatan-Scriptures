            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg hover:shadow-xl transition-shadow overflow-hidden group">
                    <div class="bg-gradient-to-br from-saffron-500 to-orange-500 p-6 text-white">
                        <div class="text-3xl font-sanskrit mb-2">{{ $veda->name_sanskrit }}</div>
                        <div class="text-lg font-semibold">{{ $veda->name_english }}</div>
                        <div class="text-sm italic opacity-90">{{ $veda->name_transliteration }}</div>
                    </div>
                    <div class="p-6">
                        <div class="space-y-2 mb-4">
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600 dark:text-gray-400">Mandalas:</span>
                                <span class="font-semibold text-gray-900 dark:text-white">{{ $veda->total_mandalas }}</span>
                            </div>
                            <div class="flex justify-between text-sm">
                                <span class="text-gray-600 dark:text-gray-400">Verses:</span>
                                <span class="font-semibold text-gray-900 dark:text-white">{{ number_format($veda->total_verses) }}</span>
                            </div>
                        </div>
